...
#'   for Data Generated Using the Vale and Maurelli (1983) Approach (Skewness = 3, Kurtosis = 21) - Ordinary Least Squares
#'
...
#' @family model fit functions
#' @keywords fit
#' @inheritParams vm_sev_fit.ols
#' @inheritParams vm_sev_dat_task
#' @export
vm_sev_fit.ols_task <- function(taskid,
                                dir = getwd(),
                                overwrite = FALSE) {
...
#' @inheritParams vm_sev_fit.ols_task
#' @inheritParams jeksterslabRpar::par_lapply
#' @inheritParams vm_sev_dat_simulation
#' @export
vm_sev_fit.ols_simulation <- function(dir = getwd(),
                                      all = TRUE,
                                      taskid = NULL,
                                      overwrite = FALSE,
                                      par = TRUE,
...
#' @export
vm_sev_fit.ols_task_summary <- function(taskid,
                                        dir = getwd()) {
  # for socks to load package in the namespace
  requireNamespace(
    "jeksterslabRmedsimple",
...
#' @family model fit functions
#' @keywords fit
#' @inheritParams vm_sev_fit.ols_simulation
#' @inheritParams vm_sev_dat_simulation
#' @export
vm_sev_fit.ols_simulation_summary <- function(dir = getwd(),
                                              all = TRUE,
                                              taskid = NULL,
                                              par = TRUE,
                                              ncores = NULL,
  ...
  out <- invisible(
    par_lapply(
      X = taskid,
      FUN = vm_sev_fit.ols_task_summary,
      dir = dir,
      par = par,
      ncores = ncores,
      blas_threads = blas_threads,
  ...
  saveRDS(
    ...
    file = file.path(
      dir,
      "summary_medsimple_vm_sev_fit.ols.Rds"
    )
  )
}
