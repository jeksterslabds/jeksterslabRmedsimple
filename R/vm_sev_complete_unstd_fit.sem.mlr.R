...
#' vm_sev_fit.sem.mlr(data = data, taskid = taskid)
#' vm_sev_fit.ols(data = data, taskid = taskid)
#' @export
vm_sev_fit.sem.mlr <- function(data,
                               taskid) {
...
#' @family model fit functions
#' @keywords fit
#' @inheritParams vm_sev_fit.sem.mlr
#' @inheritParams mvn_dat_task
#' @export
vm_sev_fit.sem.mlr_task <- function(taskid,
                                    dir = getwd(),
                                    overwrite = FALSE) {
...
#' @inheritParams vm_sev_fit.sem.mlr_task
#' @inheritParams jeksterslabRpar::par_lapply
#' @inheritParams mvn_dat_simulation
#' @export
vm_sev_fit.sem.mlr_simulation <- function(dir = getwd(),
                                          all = TRUE,
                                          taskid = NULL,
                                          overwrite = FALSE,
                                          par = TRUE,
...
#' @export
vm_sev_fit.sem.mlr_task_summary <- function(taskid,
                                            dir = getwd()) {
  # for socks to load package in the namespace
  requireNamespace(
    "jeksterslabRmedsimple",
...
#' @family model fit functions
#' @keywords fit
#' @inheritParams vm_sev_fit.sem.mlr_simulation
#' @inheritParams mvn_dat_simulation
#' @export
vm_sev_fit.sem.mlr_simulation_summary <- function(dir = getwd(),
                                                  all = TRUE,
                                                  taskid = NULL,
                                                  par = TRUE,
                                                  ncores = NULL,
  ...
  out <- invisible(
    par_lapply(
      X = taskid,
      FUN = vm_sev_fit.sem.mlr_task_summary,
      dir = dir,
      par = par,
      ncores = ncores,
      blas_threads = blas_threads,
  ...
  saveRDS(
    ...
    file = file.path(
      dir,
      "summary_medsimple_vm_sev_fit.sem.mlr.Rds"
    )
  )
}
