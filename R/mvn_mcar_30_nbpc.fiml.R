...
#' @export
mvn_mcar_30_nbpc.fiml_task <- function(taskid,
                                       dir = getwd(),
                                       overwrite = FALSE) {
  ...
  fn <- paste0(
    "medsimple_mvn_mcar_30_nbpc.fiml_",
    sprintf(
      "%05.0f",
      taskid
    ),
...
#' @export
mvn_mcar_30_nbpc.fiml_simulation <- function(dir = getwd(),
                                             all = TRUE,
                                             taskid = NULL,
                                             overwrite = FALSE,
                                             par = TRUE,
  ...
  out <- invisible(
    par_lapply(
      X = taskid,
      FUN = mvn_mcar_30_nbpc.fiml_task,
      dir = dir,
      overwrite = overwrite,
      par = par,
      ncores = ncores,
...
#' @inheritParams mvn_dat_simulation
#' @export
mvn_mcar_30_nb.fiml_pcci_simulation <- function(dir = getwd(),
                                                all = TRUE,
                                                taskid = NULL) {
  # for socks to load package in the namespace
  requireNamespace(
  ...
  out <- label(
    out = out,
    method = "NBPC.FIML.MCAR.30",
    model = "Simple Mediation Model",
    std = FALSE,
    missing = "MCAR.30"
  )
  fn <- "summary_medsimple_mvn_mcar_30_nb.fiml_pcci.Rds"
  saveRDS(
    object = out,
    file = fn
  )
