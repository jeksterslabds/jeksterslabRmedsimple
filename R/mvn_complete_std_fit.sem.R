...
#' mvn_std_fit.sem(data = data, taskid = taskid)
#' mvn_fit.ols(data = data, taskid = taskid)
#' @export
mvn_std_fit.sem <- function(data,
                            taskid) {
...
#' @export
mvn_std_fit.sem_task <- function(taskid,
                                 dir = getwd(),
                                 overwrite = FALSE) {
...
#' @export
mvn_std_fit.sem_simulation <- function(dir = getwd(),
                                       all = TRUE,
                                       taskid = NULL,
                                       overwrite = FALSE,
                                       par = TRUE,
...
#' @export
mvn_std_fit.sem_task_summary <- function(taskid,
                                         dir = getwd()) {
  # for socks to load package in the namespace
  requireNamespace(
    "jeksterslabRmedsimple",
...
#' @inheritParams mvn_dat_simulation
#' @export
mvn_std_fit.sem_simulation_summary <- function(dir = getwd(),
                                               all = TRUE,
                                               taskid = NULL,
                                               par = TRUE,
                                               ncores = NULL,
  ...
  out <- invisible(
    par_lapply(
      X = taskid,
      FUN = mvn_std_fit.sem_task_summary,
      dir = dir,
      par = par,
      ncores = ncores,
      blas_threads = blas_threads,
  ...
  saveRDS(
    ...
    file = file.path(
      dir,
      "summary_medsimple_mvn_std_fit.sem.Rds"
    )
  )
}
