...
#' mvn_std_mcmvndelta(data = data, taskid = taskid, R = 200)
#' @export
mvn_std_mcmvndelta <- function(data,
                               taskid,
                               R = 5000) {
  ...
  estimates <- .fit(
    data,
    minimal = FALSE,
    std = FALSE # this arguments doesn't really matter in this case
  )
  betahatprime <- estimates["betahatprime"]
  alphahatprime <- estimates["alphahatprime"]
  sehatbetahatprime <- estimates["sehatbetahatprime"]
  sehatalphahatprime <- estimates["sehatalphahatprime"]
  out <- .mcmvn(
    R = R,
...
#' taskid <- 1
#' data <- mvn_dat(taskid = taskid)
#' mvn_std_mctdelta(data = data, taskid = taskid, R = 200)
#' @export
mvn_std_mctdelta <- function(data,
                             taskid,
                             R = 5000) {
  paramsmvn <- useparamsmvn(taskid = taskid)
  estimates <- .fit(
...
#' @family monte carlo method functions
#' @keywords mc
#' @inheritParams mvn_std_mcmvndelta
#' @inheritParams mvn_fit_task
#' @export
mvn_std_mcmvndelta_task <- function(taskid,
                                    R = 5000,
                                    dir = getwd(),
                                    overwrite = FALSE) {
...
#' @inheritParams mvn_std_mcmvndelta_task
#' @inheritParams jeksterslabRpar::par_lapply
#' @inheritParams mvn_dat_simulation
#' @export
mvn_std_mcmvndelta_simulation <- function(dir = getwd(),
                                          all = TRUE,
                                          taskid = NULL,
                                          overwrite = FALSE,
                                          R = 5000,
...
#' @family monte carlo method functions
#' @keywords mc
#' @inheritParams mvn_std_mctdelta
#' @inheritParams mvn_fit_task
#' @export
mvn_std_mctdelta_task <- function(taskid,
                                  R = 5000,
                                  dir = getwd(),
                                  overwrite = FALSE) {
  # for socks to load package in the namespace
...
#' @keywords mc
#' @importFrom jeksterslabRpar par_lapply
#' @inheritParams mvn_std_mctdelta_task
#' @inheritParams jeksterslabRpar::par_lapply
#' @inheritParams mvn_dat_simulation
#' @export
mvn_std_mctdelta_simulation <- function(dir = getwd(),
                                        all = TRUE,
                                        taskid = NULL,
                                        overwrite = FALSE,
                                        R = 5000,
...
#' @inheritParams pcci
#' @export
mvn_std_mcmvndelta_pcci_task <- function(taskid,
                                         dir = getwd(),
...
#' @keywords mc
#' @importFrom jeksterslabRpar par_lapply
#' @inheritParams mvn_std_mcmvndelta_pcci_task
#' @inheritParams jeksterslabRpar::par_lapply
#' @inheritParams mvn_dat_simulation
#' @export
mvn_std_mcmvndelta_pcci_simulation <- function(dir = getwd(),
                                               all = TRUE,
                                               taskid = NULL,
                                               alpha = c(0.001, 0.01, 0.05),
                                               par = TRUE,
  ...
  out <- invisible(
    par_lapply(
      X = taskid,
      FUN = mvn_std_mcmvndelta_pcci_task,
      dir = dir,
      alpha = alpha,
      par = par,
      ncores = ncores,
  ...
  fn <- "summary_medsimple_mvn_std_mcmvndelta_pcci.Rds"
  saveRDS(
    object = out,
    file = fn
  )
...
#' @inheritParams pcci
#' @export
mvn_std_mctdelta_pcci_task <- function(taskid,
                                       dir = getwd(),
                                       alpha = c(0.001, 0.01, 0.05)) {
  # for socks to load package in the namespace
  requireNamespace(
...
#' @keywords mc
#' @importFrom jeksterslabRpar par_lapply
#' @inheritParams mvn_std_mctdelta_pcci_task
#' @inheritParams jeksterslabRpar::par_lapply
#' @inheritParams mvn_dat_simulation
#' @export
mvn_std_mctdelta_pcci_simulation <- function(dir = getwd(),
                                             all = TRUE,
                                             taskid = NULL,
                                             alpha = c(0.001, 0.01, 0.05),
                                             par = TRUE,
  ...
  out <- invisible(
    par_lapply(
      X = taskid,
      FUN = mvn_std_mctdelta_pcci_task,
      dir = dir,
      alpha = alpha,
      par = par,
      ncores = ncores,
  ...
  fn <- "summary_medsimple_mvn_std_mctdelta_pcci.Rds"
  saveRDS(
    object = out,
    file = fn
  )
