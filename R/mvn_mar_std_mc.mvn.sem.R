...
#' data <- mvn_dat_mar(data, taskid = taskid)
#' fit.sem(data = data, minimal = TRUE, std = TRUE, fiml = TRUE)
#'
#' fit <- mvn_mar_std_fit.sem(data = data, taskid = taskid)
#' thetahatstar <- mvn_mar_std_mc.mvn.sem(
...
#' hist(thetahatstar)
#' @export
mvn_mar_std_mc.mvn.sem <- function(taskid,
                                   R = 20000L,
                                   alphahatprime,
...
#' @export
mvn_mar_std_mc.mvn.sem_task <- function(taskid,
                                        dir = getwd(),
                                        overwrite = FALSE) {
  ...
  setwd(dir)
  fnest <- paste0(
    "medsimple_mvn_mar_std_fit.sem_",
    sprintf(
      "%05.0f",
      taskid
    ),
  ...
  fn <- paste0(
    "medsimple_mvn_mar_std_mc.mvn.sem_",
    sprintf(
      "%05.0f",
      taskid
    ),
...
#' @inheritParams mvn_mar_std_mc.mvn.sem_task
#' @inheritParams jeksterslabRpar::par_lapply
#' @inheritParams mvn_dat_simulation
#' @export
mvn_mar_std_mc.mvn.sem_simulation <- function(dir = getwd(),
                                              all = TRUE,
                                              taskid = NULL,
                                              overwrite = FALSE,
                                              par = TRUE,
  ...
  out <- invisible(
    par_lapply(
      X = taskid,
      FUN = mvn_mar_std_mc.mvn.sem_task,
      dir = dir,
      overwrite = overwrite,
      par = par,
      ncores = ncores,
...
#' @inheritParams mvn_dat_task
#' @export
mvn_mar_std_mc.mvn.sem_pcci_task <- function(taskid,
                                             dir = getwd()) {
  # for socks to load package in the namespace
  requireNamespace(
    "jeksterslabRmedsimple",
...
#' @inheritParams mvn_dat_simulation
#' @export
mvn_mar_std_mc.mvn.sem_pcci_simulation <- function(dir = getwd(),
                                                   all = TRUE,
                                                   taskid = NULL,
                                                   par = TRUE,
                                                   ncores = NULL,
  ...
  out <- invisible(
    par_lapply(
      X = taskid,
      FUN = mvn_mar_std_mc.mvn.sem_pcci_task,
      dir = dir,
      par = par,
      ncores = ncores,
      blas_threads = blas_threads,
  ...
  fn <- "summary_medsimple_mvn_mar_std_mc.mvn.sem_pcci.Rds"
  saveRDS(
    object = out,
    file = fn
  )
