...
#'
#' @title Monte Carlo Method Assuming Student's t Distribution using Delta Method Standard Errors for Indirect Effect in a Standardized Simple Mediation Model
...
#' thetahatstar <- mvn_std_mc.t.delta(
...
#' hist(thetahatstar)
#' @export
mvn_std_mc.t.delta <- function(taskid,
                               R = 20000L,
                               alphahatprime,
...
#' @export
mvn_std_mc.t.delta_task <- function(taskid,
                                    dir = getwd(),
                                    overwrite = FALSE) {
...
#' @inheritParams mvn_std_mc.t.delta_task
#' @inheritParams jeksterslabRpar::par_lapply
#' @inheritParams mvn_dat_simulation
#' @export
mvn_std_mc.t.delta_simulation <- function(dir = getwd(),
                                          all = TRUE,
                                          taskid = NULL,
                                          overwrite = FALSE,
                                          par = TRUE,
...
#' @author Ivan Jacob Agaloos Pesigan
#'
#' @title Monte Carlo Method Confidence Intervals Assuming Student's t Distribution using Delta Method Standard Errors for Indirect Effect in a Standardized Simple Mediation Model
#'   for Data Generated from a Multivariate Normal Distribution
#'   (Single Task)
#'
#' @family monte carlo method functions
...
#' @inheritParams mvn_dat_task
#' @export
mvn_std_mc.t.delta_pcci_task <- function(taskid,
                                         dir = getwd()) {
  # for socks to load package in the namespace
  requireNamespace(
    "jeksterslabRmedsimple",
...
#' @inheritParams mvn_dat_simulation
#' @export
mvn_std_mc.t.delta_pcci_simulation <- function(dir = getwd(),
                                               all = TRUE,
                                               taskid = NULL,
                                               par = TRUE,
                                               ncores = NULL,
  ...
  out <- invisible(
    par_lapply(
      X = taskid,
      FUN = mvn_std_mc.t.delta_pcci_task,
      dir = dir,
      par = par,
      ncores = ncores,
      blas_threads = blas_threads,
  ...
  fn <- "summary_medsimple_mvn_std_mc.t.delta_pcci.Rds"
  saveRDS(
    object = out,
    file = fn
  )
