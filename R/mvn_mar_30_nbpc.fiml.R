...
#' @export
mvn_mar_30_nbpc.fiml_task <- function(taskid,
                                      dir = getwd(),
                                      overwrite = FALSE) {
  ...
  fn <- paste0(
    "medsimple_mvn_mar_30_nbpc.fiml_",
    sprintf(
      "%05.0f",
      taskid
    ),
...
#' @export
mvn_mar_30_nbpc.fiml_simulation <- function(dir = getwd(),
                                            all = TRUE,
                                            taskid = NULL,
                                            overwrite = FALSE,
                                            par = TRUE,
...
#' @inheritParams mvn_dat_simulation
#' @export
mvn_mar_30_nb.fiml_pcci_simulation <- function(dir = getwd(),
                                               all = TRUE,
                                               taskid = NULL) {
  # for socks to load package in the namespace
  requireNamespace(
  ...
  out <- label(
    out = out,
    method = "NBPC.FIML.MAR.30",
    model = "Simple Mediation Model",
    std = FALSE,
    missing = "MAR.30"
  )
  fn <- "summary_medsimple_mvn_mar_30_nb.fiml_pcci.Rds"
  saveRDS(
    object = out,
    file = fn
  )
