...
#' hist(thetahatstar)
#' @export
vm_mod_ols_mc.mvn <- function(taskid,
                              R = 20000L,
                              alphahat,
                              sehatalphahat,
                              betahat,
...
#' @export
vm_mod_ols_mc.mvn_task <- function(taskid,
                                   dir = getwd(),
                                   overwrite = FALSE) {
...
#' @inheritParams vm_mod_ols_mc.mvn_task
#' @inheritParams jeksterslabRpar::par_lapply
#' @inheritParams vm_mod_dat_simulation
#' @export
vm_mod_ols_mc.mvn_simulation <- function(dir = getwd(),
                                         all = TRUE,
                                         taskid = NULL,
                                         overwrite = FALSE,
                                         par = TRUE,
...
#' @inheritParams vm_mod_dat_task
#' @export
vm_mod_ols_mc.mvn_pcci_task <- function(taskid,
                                        dir = getwd()) {
  # for socks to load package in the namespace
  requireNamespace(
    "jeksterslabRmedsimple",
...
#' @inheritParams vm_mod_dat_simulation
#' @export
vm_mod_ols_mc.mvn_pcci_simulation <- function(dir = getwd(),
                                              all = TRUE,
                                              taskid = NULL,
                                              par = TRUE,
                                              ncores = NULL,
  ...
  out <- invisible(
    par_lapply(
      X = taskid,
      FUN = vm_mod_ols_mc.mvn_pcci_task,
      dir = dir,
      par = par,
      ncores = ncores,
      blas_threads = blas_threads,
  ...
  fn <- "summary_medsimple_vm_mod_ols_mc.mvn_pcci.Rds"
  saveRDS(
    object = out,
    file = fn
  )
