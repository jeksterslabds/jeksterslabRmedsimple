...
#' exp_fit.sem.mlr(data = data, taskid = taskid)
#' exp_fit.ols(data = data, taskid = taskid)
#' @export
exp_fit.sem.mlr <- function(data,
                            taskid) {
...
#' @family model fit functions
#' @keywords fit
#' @inheritParams exp_fit.sem.mlr
#' @inheritParams exp_dat_task
#' @export
exp_fit.sem.mlr_task <- function(taskid,
                                 dir = getwd(),
                                 overwrite = FALSE) {
...
#' @inheritParams exp_fit.sem.mlr_task
#' @inheritParams jeksterslabRpar::par_lapply
#' @inheritParams exp_dat_simulation
#' @export
exp_fit.sem.mlr_simulation <- function(dir = getwd(),
                                       all = TRUE,
                                       taskid = NULL,
                                       overwrite = FALSE,
                                       par = TRUE,
...
#' @export
exp_fit.sem.mlr_task_summary <- function(taskid,
                                         dir = getwd()) {
  # for socks to load package in the namespace
  requireNamespace(
    "jeksterslabRmedsimple",
...
#' @family model fit functions
#' @keywords fit
#' @inheritParams exp_fit.sem.mlr_simulation
#' @inheritParams exp_dat_simulation
#' @export
exp_fit.sem.mlr_simulation_summary <- function(dir = getwd(),
                                               all = TRUE,
                                               taskid = NULL,
                                               par = TRUE,
                                               ncores = NULL,
  ...
  out <- invisible(
    par_lapply(
      X = taskid,
      FUN = exp_fit.sem.mlr_task_summary,
      dir = dir,
      par = par,
      ncores = ncores,
      blas_threads = blas_threads,
  ...
  saveRDS(
    ...
    file = file.path(
      dir,
      "summary_medsimple_exp_fit.sem.mlr.Rds"
    )
  )
}
