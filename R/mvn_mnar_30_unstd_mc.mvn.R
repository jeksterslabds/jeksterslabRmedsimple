...
#'   for Data Generated from a Multivariate Normal Distribution with Data Missing Not at Random - 30%
#'
...
#'
#' fit <- mvn_mnar_30_fit.sem(data = data, taskid = taskid)
#' thetahatstar <- mvn_mnar_30_mc.mvn(
...
#' hist(thetahatstar)
#' @export
mvn_mnar_30_mc.mvn <- function(taskid,
                               R = 20000L,
                               alphahat,
                               sehatalphahat,
                               betahat,
...
#' @export
mvn_mnar_30_mc.mvn_task <- function(taskid,
                                    dir = getwd(),
                                    overwrite = FALSE) {
...
#' @inheritParams mvn_mnar_30_mc.mvn_task
#' @inheritParams jeksterslabRpar::par_lapply
#' @inheritParams mvn_dat_simulation
#' @export
mvn_mnar_30_mc.mvn_simulation <- function(dir = getwd(),
                                          all = TRUE,
                                          taskid = NULL,
                                          overwrite = FALSE,
                                          par = TRUE,
...
#' @inheritParams mvn_dat_task
#' @export
mvn_mnar_30_mc.mvn_pcci_task <- function(taskid,
                                         dir = getwd()) {
  # for socks to load package in the namespace
  requireNamespace(
    "jeksterslabRmedsimple",
...
#' @inheritParams mvn_dat_simulation
#' @export
mvn_mnar_30_mc.mvn_pcci_simulation <- function(dir = getwd(),
                                               all = TRUE,
                                               taskid = NULL,
                                               par = TRUE,
                                               ncores = NULL,
  ...
  out <- invisible(
    par_lapply(
      X = taskid,
      FUN = mvn_mnar_30_mc.mvn_pcci_task,
      dir = dir,
      par = par,
      ncores = ncores,
      blas_threads = blas_threads,
  ...
  fn <- "summary_medsimple_mvn_mnar_30_mc.mvn_pcci.Rds"
  saveRDS(
    object = out,
    file = fn
  )
