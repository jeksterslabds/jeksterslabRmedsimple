...
#' @export
mvn_mnar_10_nbpc.fiml_task <- function(taskid,
                                       dir = getwd(),
                                       overwrite = FALSE) {
  ...
  fn <- paste0(
    "medsimple_mvn_mnar_10_nbpc.fiml_",
    sprintf(
      "%05.0f",
      taskid
    ),
...
#' @export
mvn_mnar_10_nbpc.fiml_simulation <- function(dir = getwd(),
                                             all = TRUE,
                                             taskid = NULL,
                                             overwrite = FALSE,
                                             par = TRUE,
  ...
  out <- invisible(
    par_lapply(
      X = taskid,
      FUN = mvn_mnar_10_nbpc.fiml_task,
      dir = dir,
      overwrite = overwrite,
      par = par,
      ncores = ncores,
...
mvn_mnar_10_nb.fiml_pcci_simulation <- function(dir = getwd(),
  ...
  out <- label(
    out = out,
    method = "NBPC.FIML.MNAR.10",
    model = "Simple Mediation Model",
    std = FALSE,
    missing = "MNAR.10"
  )
  fn <- "summary_medsimple_mvn_mnar_10_nb.fiml_pcci.Rds"
  saveRDS(
    object = out,
    file = fn
  )
