...
#'
#' # mvn_mar_50_fit.sem -------------------------------------------------
#' mvn_mar_50_fit.sem(data = data, taskid = taskid)
#'
#' # mvn_fit.ols for comparison ---------------------------------------
#' mvn_fit.ols(data = data[complete.cases(data), ], taskid = taskid)
#'
#' # mvn_mar_50_fit.sem with missing data -------------------------------
#' mar_50 <- mvn_dat_mar_50(data = data, taskid = taskid)
#' mvn_mar_50_fit.sem(data = mar_50, taskid = taskid)
#' @export
mvn_mar_50_fit.sem <- function(data,
                               taskid) {
...
#' @family model fit functions
#' @keywords mvn_mar_unstd
#' @inheritParams mvn_mar_50_fit.sem
#' @inheritParams mvn_dat_task
#' @export
mvn_mar_50_fit.sem_task <- function(taskid,
                                    dir = getwd(),
                                    overwrite = FALSE) {
...
#' @inheritParams mvn_mar_50_fit.sem_task
#' @inheritParams jeksterslabRpar::par_lapply
#' @inheritParams mvn_dat_simulation
#' @export
mvn_mar_50_fit.sem_simulation <- function(dir = getwd(),
                                          all = TRUE,
                                          taskid = NULL,
                                          overwrite = FALSE,
                                          par = TRUE,
...
#' @export
mvn_mar_50_fit.sem_task_summary <- function(taskid,
                                            dir = getwd()) {
  # for socks to load package in the namespace
  requireNamespace(
    "jeksterslabRmedsimple",
...
#' @family model fit functions
#' @keywords mvn_mar_unstd
#' @inheritParams mvn_mar_50_fit.sem_simulation
#' @inheritParams mvn_dat_simulation
#' @export
mvn_mar_50_fit.sem_simulation_summary <- function(dir = getwd(),
                                                  all = TRUE,
                                                  taskid = NULL,
                                                  par = TRUE,
                                                  ncores = NULL,
  ...
  out <- invisible(
    par_lapply(
      X = taskid,
      FUN = mvn_mar_50_fit.sem_task_summary,
      dir = dir,
      par = par,
      ncores = ncores,
      blas_threads = blas_threads,
  ...
  saveRDS(
    ...
    file = file.path(
      dir,
      "summary_medsimple_mvn_mar_50_fit.sem.Rds"
    )
  )
}
