...
#'   for Data Generated from a Multivariate Normal Distribution with Data Missing at Random - 30%
#'
...
#' data <- mvn_mar_30_dat(data, taskid = taskid)
#' fit.sem(data = data, minimal = TRUE, std = FALSE, fiml = TRUE)
#'
#' fit <- mvn_mar_30_fit.sem(data = data, taskid = taskid)
#' thetahatstar <- mvn_mar_30_mc.mvn(
...
#' hist(thetahatstar)
#' @export
mvn_mar_30_mc.mvn <- function(taskid,
                              R = 20000L,
                              alphahat,
                              sehatalphahat,
                              betahat,
...
#' @export
mvn_mar_30_mc.mvn_task <- function(taskid,
                                   dir = getwd(),
                                   overwrite = FALSE) {
...
#' @inheritParams mvn_mar_30_mc.mvn_task
#' @inheritParams jeksterslabRpar::par_lapply
#' @inheritParams mvn_dat_simulation
#' @export
mvn_mar_30_mc.mvn_simulation <- function(dir = getwd(),
                                         all = TRUE,
                                         taskid = NULL,
                                         overwrite = FALSE,
                                         par = TRUE,
...
#' @inheritParams mvn_dat_task
#' @export
mvn_mar_30_mc.mvn_pcci_task <- function(taskid,
                                        dir = getwd()) {
  # for socks to load package in the namespace
  requireNamespace(
    "jeksterslabRmedsimple",
...
#' @inheritParams mvn_dat_simulation
#' @export
mvn_mar_30_mc.mvn_pcci_simulation <- function(dir = getwd(),
                                              all = TRUE,
                                              taskid = NULL,
                                              par = TRUE,
                                              ncores = NULL,
  ...
  out <- invisible(
    par_lapply(
      X = taskid,
      FUN = mvn_mar_30_mc.mvn_pcci_task,
      dir = dir,
      par = par,
      ncores = ncores,
      blas_threads = blas_threads,
  ...
  fn <- "summary_medsimple_mvn_mar_30_mc.mvn_pcci.Rds"
  saveRDS(
    object = out,
    file = fn
  )
