...
#'
#' fit <- beta_fit.ols(data = data, taskid = taskid)
#' thetahatstar <- beta_ols_mc.mvn(
...
#' hist(thetahatstar)
#' @export
beta_ols_mc.mvn <- function(taskid,
                            R = 20000L,
                            alphahat,
                            sehatalphahat,
                            betahat,
...
#' @export
beta_ols_mc.mvn_task <- function(taskid,
                                 dir = getwd(),
                                 overwrite = FALSE) {
...
#' @inheritParams beta_ols_mc.mvn_task
#' @inheritParams jeksterslabRpar::par_lapply
#' @inheritParams beta_dat_simulation
#' @export
beta_ols_mc.mvn_simulation <- function(dir = getwd(),
                                       all = TRUE,
                                       taskid = NULL,
                                       overwrite = FALSE,
                                       par = TRUE,
...
#' @inheritParams beta_dat_task
#' @export
beta_ols_mc.mvn_pcci_task <- function(taskid,
                                      dir = getwd()) {
  # for socks to load package in the namespace
  requireNamespace(
    "jeksterslabRmedsimple",
...
#' @inheritParams beta_dat_simulation
#' @export
beta_ols_mc.mvn_pcci_simulation <- function(dir = getwd(),
                                            all = TRUE,
                                            taskid = NULL,
                                            par = TRUE,
                                            ncores = NULL,
  ...
  fn <- "summary_medsimple_beta_ols_mc.mvn_pcci.Rds"
  saveRDS(
    object = out,
    file = fn
  )
