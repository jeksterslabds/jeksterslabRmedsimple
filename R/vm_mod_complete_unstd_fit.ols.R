...
#'   for Data Generated Using the Vale and Maurelli (1983) Approach (Skewness = 2, Kurtosis = 7) - Ordinary Least Squares
#'
...
#' vm_mod_fit.ols(data = data, taskid = taskid)
#' @export
vm_mod_fit.ols <- function(data,
                           taskid) {
...
#' @family model fit functions
#' @keywords fit
#' @inheritParams vm_mod_fit.ols
#' @inheritParams vm_mod_dat_task
#' @export
vm_mod_fit.ols_task <- function(taskid,
                                dir = getwd(),
                                overwrite = FALSE) {
...
#' @inheritParams vm_mod_fit.ols_task
#' @inheritParams jeksterslabRpar::par_lapply
#' @inheritParams vm_mod_dat_simulation
#' @export
vm_mod_fit.ols_simulation <- function(dir = getwd(),
                                      all = TRUE,
                                      taskid = NULL,
                                      overwrite = FALSE,
                                      par = TRUE,
...
#' @export
vm_mod_fit.ols_task_summary <- function(taskid,
                                        dir = getwd()) {
  # for socks to load package in the namespace
  requireNamespace(
    "jeksterslabRmedsimple",
...
#' @family model fit functions
#' @keywords fit
#' @inheritParams vm_mod_fit.ols_simulation
#' @inheritParams vm_mod_dat_simulation
#' @export
vm_mod_fit.ols_simulation_summary <- function(dir = getwd(),
                                              all = TRUE,
                                              taskid = NULL,
                                              par = TRUE,
                                              ncores = NULL,
  ...
  out <- invisible(
    par_lapply(
      X = taskid,
      FUN = vm_mod_fit.ols_task_summary,
      dir = dir,
      par = par,
      ncores = ncores,
      blas_threads = blas_threads,
  ...
  saveRDS(
    ...
    file = file.path(
      dir,
      "summary_medsimple_vm_mod_fit.ols.Rds"
    )
  )
}
