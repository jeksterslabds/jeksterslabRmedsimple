...
#'
#' # mvn_mcar_50_fit.sem -------------------------------------------------
#' mvn_mcar_50_fit.sem(data = data, taskid = taskid)
#'
#' # mvn_fit.ols for comparison ---------------------------------------
#' mvn_fit.ols(data = data[complete.cases(data), ], taskid = taskid)
#'
#' # mvn_mcar_50_fit.sem with missing data -------------------------------
#' mcar_50 <- mvn_dat_mcar_50(data = data, taskid = taskid)
#' mvn_mcar_50_fit.sem(data = mcar_50, taskid = taskid)
#' @export
mvn_mcar_50_fit.sem <- function(data,
                                taskid) {
...
#' @family model fit functions
#' @keywords mvn_mcar_unstd
#' @inheritParams mvn_mcar_50_fit.sem
#' @inheritParams mvn_dat_task
#' @export
mvn_mcar_50_fit.sem_task <- function(taskid,
                                     dir = getwd(),
                                     overwrite = FALSE) {
  ...
  fn <- paste0(
    "medsimple_mvn_mcar_50_fit.sem_",
    sprintf(
      "%05.0f",
      taskid
    ),
...
#' @inheritParams mvn_mcar_50_fit.sem_task
#' @inheritParams jeksterslabRpar::par_lapply
#' @inheritParams mvn_dat_simulation
#' @export
mvn_mcar_50_fit.sem_simulation <- function(dir = getwd(),
                                           all = TRUE,
                                           taskid = NULL,
                                           overwrite = FALSE,
                                           par = TRUE,
...
#' @export
mvn_mcar_50_fit.sem_task_summary <- function(taskid,
                                             dir = getwd()) {
  # for socks to load package in the namespace
  requireNamespace(
    "jeksterslabRmedsimple",
...
#' @family model fit functions
#' @keywords mvn_mcar_unstd
#' @inheritParams mvn_mcar_50_fit.sem_simulation
#' @inheritParams mvn_dat_simulation
#' @export
mvn_mcar_50_fit.sem_simulation_summary <- function(dir = getwd(),
                                                   all = TRUE,
                                                   taskid = NULL,
                                                   par = TRUE,
                                                   ncores = NULL,
  ...
  out <- invisible(
    par_lapply(
      X = taskid,
      FUN = mvn_mcar_50_fit.sem_task_summary,
      dir = dir,
      par = par,
      ncores = ncores,
      blas_threads = blas_threads,
  ...
  saveRDS(
    ...
    file = file.path(
      dir,
      "summary_medsimple_mvn_mcar_50_fit.sem.Rds"
    )
  )
}
